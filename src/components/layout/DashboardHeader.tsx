...
const DashboardHeader = ({
  ...
  const [searchResults, setSearchResults] = useState<SearchPickerResult[]>([]);
  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    try {
      const response = await searchApi.searchPickers(query, undefined, undefined, 1, 10);
  ...
  const handlePickerClick = (pickerResult: SearchPickerResult) => {
    // Transform search result to match JetPickerDetails Picker interface
    if (pickerResult.travelJourneys && pickerResult.travelJourneys.length > 0) {
      const journey = pickerResult.travelJourneys[0];
      const picker = {
        id: journey.id,
        picker: {
          id: pickerResult.id,
          full_name: pickerResult.full_name,
          avatar_url: pickerResult.avatar_url || '',
          rating: pickerResult.rating,
          completed_deliveries: pickerResult.completed_orders,
        },
      ...
      navigate('/orderer/jetpicker-details', { state: { picker } });
    }
    setSearchQuery('');
    setShowSearchResults(false);
  };
              ...
              <div className="divide-y divide-gray-100">
                {searchResults.map((picker) => (
                  ...
                  >
                    {picker.avatar_url ? (
                      <img
                        src={imageUtils.getImageUrl(picker.avatar_url)}
                        alt={picker.full_name}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-900 text-sm truncate">{picker.full_name}</p>
                      <p className="text-xs text-gray-500">⭐ {picker.rating} • {picker.completed_orders} deliveries</p>
                      {picker.travelJourneys && picker.travelJourneys.length > 0 && (
                        <p className="text-xs text-gray-600 mt-1 truncate">
                          {picker.travelJourneys[0].departure_city} → {picker.travelJourneys[0].arrival_city}
                        </p>
                      )}
                    </div>
                  </button>
