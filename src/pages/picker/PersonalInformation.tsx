...
const PersonalInformation = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        setLoading(true);
        const response = await pickerProfileApi.getProfile();
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.password.trim()) {
        await pickerProfileApi.changePassword(formData.password, formData.password_confirmation);
      }
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-24 md:pb-0 bg-white">
          <div className="max-w-2xl mx-auto">
                ...
                <>
                  ...
                  <div className="bg-pink-100 rounded-lg p-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <label className="text-xs text-[#4D0013] font-medium block leading-none">Full Name</label>
                      <input
                        type="text"
                        name="full_name"
                  ...
                  <div className="bg-pink-100 rounded-lg p-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <label className="text-xs text-[#4D0013] font-medium block leading-none">New Password</label>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-2 text-[#4D0013] hover:opacity-70 transition-opacity flex-shrink-0 cursor-pointer"
                        >
                          {showPassword ? (
                  ...
                  {formData.password && (
                    <div className="bg-pink-100 rounded-lg p-3 flex items-center gap-3">
                      <Lock size={20} className="text-[#4D0013] flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <label className="text-xs text-[#4D0013] font-medium block leading-none">Confirm Password</label>
                        <input
                          type={showPassword ? 'text' : 'password'}
                  ...
                  <div className="bg-pink-100 rounded-lg p-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <label className="text-xs text-[#4D0013] font-medium block leading-none mb-2">Languages</label>
                        ...
                        >
                          {formData.languages.length > 0 ? (
                            formData.languages.map(lang => (
                              <div
                                key={lang}
                                className="bg-[#4D0013] text-white px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2"
                              >
                                {lang}
                                <button
                                  onClick={(e) => {
                          ...
                          <ChevronDown size={16} className={`text-[#4D0013] ml-auto transition-transform ${isLanguageDropdownOpen ? 'rotate-180' : ''}`} />
                        </div>
                        ...
                        {isLanguageDropdownOpen && (
                          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-300 rounded-lg shadow-lg z-20 max-h-48 overflow-y-auto">
                            {availableLanguages.map(lang => (
                              ...
                              >
                                {formData.languages.includes(lang) && (
                                  <svg className="w-4 h-4 text-[#4D0013]" fill="currentColor" viewBox="0 0 20 20">
            ...
            <button
              ...
              className="bg-[#C41E3A] text-white py-3 px-16 rounded-lg font-bold text-base hover:bg-[#A01830] transition-colors max-w-sm mx-auto block disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {saving ? 'Saving...' : 'Save'}
            </button>
          </div>
